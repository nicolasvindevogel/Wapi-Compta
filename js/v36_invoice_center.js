...
(function(){
  ...
  const store={tab:localStorage.getItem('wapi_v36_invoice_tab')||'to_process',manager:localStorage.getItem('wapi_v36_invoice_manager')||'',copro:localStorage.getItem('wapi_v36_invoice_copro')||'',search:'',links:[],busy:false,selected:new Set()};
  ...
  function readForm(){
    ...
    return {copro_id:$('v36FieldCopro')?.value||null,supplier_id:$('v36FieldSupplier')?.value||null,account_id:$('v36FieldAccount')?.value||null,reference:$('v36FieldReference')?.value.trim()||null,date:$('v36FieldDate')?.value||null,due_date:$('v36FieldDue')?.value||null,amount_excl_vat:tax.ht,vat_amount:tax.vat,amount,vat_rate,description:$('v36FieldDescription')?.value.trim()||null,notes:$('v36FieldNotes')?.value.trim()||null};
  }
  ...
  document.addEventListener('change',e=>{if(e.target.id==='v36SelectVisible'){visibleRows().filter(bulkEligible).forEach(q=>e.target.checked?store.selected.add(String(q.id)):store.selected.delete(String(q.id)));render();return;}const selected=e.target.closest?.('[data-v36-select]');if(selected){selected.checked?store.selected.add(String(selected.dataset.v36Select)):store.selected.delete(String(selected.dataset.v36Select));render();return;}if(e.target.id==='v36Manager'){store.manager=e.target.value;store.copro='';localStorage.setItem('wapi_v36_invoice_manager',store.manager);localStorage.removeItem('wapi_v36_invoice_copro');state.ocrSelectedQueueId='';render();}if(e.target.id==='v36Copro'){store.copro=e.target.value;localStorage.setItem('wapi_v36_invoice_copro',store.copro);state.ocrSelectedQueueId='';render();}if(e.target.id==='v36FieldCopro'){const q=selectedQ(),form=readForm(),current=values(q),keptSupplier=form.supplier_id||current.supplier_id||qSupplier(q)||null,next={...current,...form,copro_id:e.target.value||null,supplier_id:keptSupplier,amount:form.amount??current.amount};patchQueue(q.id,{copro_id:e.target.value||null,corrected_data:next});render();}if(e.target.id==='v36FieldSupplier'){const q=selectedQ(),next={...values(q),...readForm(),supplier_id:e.target.value||null};patchQueue(q.id,{corrected_data:next});render();}const toggle=e.target.closest?.('[data-v36-supplier-copro]');if(toggle){toggle.disabled=true;(async()=>{if(toggle.checked)await ensureLink(toggle.dataset.v36SupplierCopro,toggle.dataset.supplier,'manual');else{const l=store.links.find(x=>String(x.copro_id)===String(toggle.dataset.v36SupplierCopro)&&String(x.supplier_id)===String(toggle.dataset.supplier));if(l){const r=await supabaseClient.from('compta_copro_suppliers').update({active:false,updated_at:new Date().toISOString()}).eq('id',l.id);if(r.error)alert(r.error.message);else l.active=false;}}toggle.disabled=false;toggle.closest('.v359-copro-link')?.classList.toggle('is-linked',toggle.checked);})();}});
